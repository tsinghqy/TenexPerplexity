...
Guidelines for organizing code, writing functions, and managing complexity.
...
```typescript
// ✅ Good: Small functions with self-describing names - no comments needed
function calculateTotalPriceWithTaxAndDiscount(
  basePrice: number,
  taxRate: number,
  discountPercentage: number
): number {
  const priceAfterDiscount = applyDiscountToPrice(
    basePrice,
    discountPercentage
  );
  const finalPriceWithTax = addTaxToPrice(priceAfterDiscount, taxRate);
...
// ✅ Good: Blocks are one line - function calls
function processUserRegistration(
  userData: UserRegistrationData
): Promise<RegistrationResult> {
  if (!isValidUserRegistrationData(userData)) {
    return createInvalidRegistrationResult("Invalid user data");
  }

  if (await isEmailAddressAlreadyRegistered(userData.emailAddress)) {
    return createInvalidRegistrationResult("Email already registered");
  }

  const newUser = await createUserAccountInDatabase(userData);
  await sendWelcomeEmailToNewUser(newUser.id);

  return createSuccessfulRegistrationResult(newUser);
}

function isValidUserRegistrationData(data: UserRegistrationData): boolean {
  return (
    hasRequiredRegistrationFields(data) &&
    hasValidEmailFormat(data.emailAddress)
  );
}

function hasRequiredRegistrationFields(data: UserRegistrationData): boolean {
  return data.emailAddress !== "" && data.password !== "" && data.name !== "";
}

function hasValidEmailFormat(emailAddress: string): boolean {
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  return emailRegex.test(emailAddress);
}

// ❌ Bad: Large function with comments explaining what blocks do
function calculateTotalPriceWithTaxAndDiscount(
  basePrice: number,
  taxRate: number,
  discountPercentage: number
  ...
  // Then add tax
  const finalPriceWithTax = priceAfterDiscount * (1 + taxRate);

  return finalPriceWithTax;
}

// ❌ Bad: Function that needs comments to explain what it does
function processUserRegistration(
  userData: UserRegistrationData
): Promise<RegistrationResult> {
  ...
  const emailCheck = await fetch(
    `/api/users/check-email?email=${userData.emailAddress}`
  );
  ...
  // Send welcome email
  await fetch(`/api/users/${newUser.id}/send-welcome-email`, {
    method: "POST",
  });
...
```typescript
// ✅ Good: Early exits/guard clauses reduce nesting
function processUserMessage(
  ...
  // Main logic flows linearly without nesting
  const sanitizedMessage = sanitizeMessageContent(message);
  const messageWithMetadata = addMessageMetadata(sanitizedMessage, userId);
  return saveMessageToChat(chatId, messageWithMetadata);
}

// ✅ Good: Invert conditions to avoid nesting - check failure first, return early
async function processApiResponse(
  response: ApiResponse
): Promise<ProcessResult> {
  // Check failure condition first, return early
  if (!response.success) {
    return handleApiError(response.error);
  }

  // Check if data exists, return early if not
  if (!response.data) {
    return { success: false, error: "No data received" };
  }

  // Main logic flows linearly without nesting - happy path
  const processedData = transformResponseData(response.data);
  const savedResult = await saveProcessedData(processedData);
  return { success: true, data: savedResult };
}

// ❌ Bad: Nested logic when success condition is checked first
async function processApiResponse(
  response: ApiResponse
): Promise<ProcessResult> {
  if (response.success) {
    // Nested inside success check
    if (response.data) {
      // More nesting
      const processedData = transformResponseData(response.data);
      const savedResult = await saveProcessedData(processedData);
...
// ✅ Good: Use continue/break to avoid nested conditions in loops
function processValidMessages(messages: Message[]): ProcessedMessage[] {
  const processedMessages: ProcessedMessage[] = [];

  for (const message of messages) {
    // Early continue - skip invalid messages
    if (!isValidMessage(message)) {
      continue;
    }

    // Early continue - skip messages that are too old
    if (isMessageTooOld(message)) {
      continue;
    }

    // Main processing logic - no nesting needed
    const processed = transformMessage(message);
...
// ✅ Good: Optional chaining and nullish coalescing reduce nesting
...
// ✅ Good: No else needed after return
...
function getUserStatus(user: User): string {
  if (user.isActive) {
    ...
    // Unnecessary else - code after if is already the else case
    return "Inactive";
  }
}
```
